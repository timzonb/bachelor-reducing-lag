...
while i < len(df_client['ActualTime']):
    while df_server['ActualTime'][i - serAdd] > df_client['ActualTime'][i - sameIndex]:
        serAdd = serAdd + 1
...
plt.scatter(x, y, alpha=0.1, color='m')

plt.axhline(y=0.0, color='k', linestyle='-')

plt.scatter(x_timeDifference, y_timeDifference, alpha=0.1, color='g', label='time difference')
plt.scatter(x_Server, y_Server, alpha=0.1, color='b', label='server')
plt.scatter(x_Client, y_Client, alpha=0.1, color='r', label='client')
